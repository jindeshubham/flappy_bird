...
class network():
	...
	def net(self):
		...
		with tf.variable_scope(self.name) as scope:
			...
			o_c1 = tf.layers.conv2d(self.input_state, 32, 2,strides=4, padding="SAME", name="conv1")
			o_c1 = tf.nn.max_pool(o_c1, ksize = [1, 2, 2, 1], strides = [1, 2, 2, 1], padding = "SAME")
			o_c2 = tf.layers.conv2d(o_c1, 64, 2,strides=2, padding="SAME",  name="conv2")
			o_c3 = tf.layers.conv2d(o_c2, 64, 2,strides=1, padding="SAME",  name="conv3")

			shape = o_c3.get_shape().as_list()
			o_c3 = tf.reshape(o_c3,[-1, shape[1]*shape[2]*shape[3]])

			o_l1 = tf.nn.relu(tf.layers.dense(o_c3, 128))

			self.q_values = tf.layers.dense(o_l1,  2)
			print (self.q_values)
